                ...
                <Reorder.Group axis="y" values={inputs} onReorder={setInputs}>
                    {
                        inputs.map((option, index) => {
                            return(
                                <Reorder.Item className="mb-1 flex gap-1 flex-col" key={option.id} value={option}>
                                    <div className="col-span-12 sm:col-span-12 flex flex-row mt-1">
                                        <input
                                            ...
                                            className={"bg-gray-50 p-2 border focus:outline-none focus:border focus:border-gray-800 block ring-0 focus:ring-0 px-3 text-gray-900 text-sm w-full border-gray-300 shadow-sm focus:ring-gray-500 sm:text-sm " + (inputs.length > 2 ? "rounded-l-lg" : "rounded-lg")}
                                        />
                                        {
                                            inputs.length > 2 &&
                                            <button
